...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Completed quiz session not found';
  END IF;
END;
$function$;

REVOKE ALL ON FUNCTION public.quiz_aula_magna_cta_click(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.quiz_aula_magna_cta_click(uuid, text) TO anon, authenticated, service_role;
...
DECLARE
  v_from timestamptz;
  v_result jsonb;
  v_password_hash constant text := '$2y$12$A9rlMrxGS585LwWhd6Qoq.tor33L64CJaC0x1SANKnu49yNnvwKua';
BEGIN
  ...
  score_distribution AS (
    SELECT scores.score,
      count(fs.session_id)::int AS completions,
  ...
  cta AS (
    SELECT
      count(*) FILTER (WHERE cta_clicked_at IS NOT NULL)::int AS unique_clicks,
