...
export const Route = createFileRoute("/quiz-aula-magna")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Teste clínico com 7 decisões sobre hipotensão, hemorragia e HELLP na emergência obstétrica." },
      ...
      {stage === "question" && (
        <section className="quiz-stage" aria-labelledby="question-title">
          <header className="quiz-header">
            <BrandLockup compact />
            <div className="progress-copy"><span>Seu raciocínio em ação</span><strong>{current + 1}/{questions.length}</strong></div>
          </header>
          <div className="progress-bar" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={((current + 1) / questions.length) * 100} aria-label={`Progresso: pergunta ${current + 1} de ${questions.length}`}><span style={{ width: `${((current + 1) / questions.length) * 100}%` }} /></div>
      ...
      {stage === "result" && (
        <section className="result-stage" aria-labelledby="result-title">
          ...
          <article className="result-card">
            ...
            <div className="result-score"><strong>{score}</strong><span>/{questions.length}</span></div>
